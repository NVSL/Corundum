...
if $all || $scalei; then
    for r in ${rs[@]}; do
        for c in ${cs[@]}; do
            ...
            perf stat -o $dir_path/outputs/wc/i-$r-$c.out -C 0-$(($r+$c-1)) $dir_path/../target/release/examples/grep -r $r -c $c -f $pool $dir_path/files.list > $dir_path/outputs/wc/i-$r-$c.res
        done
    done
    echo
fi
