...
pub fn derive_pclone(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    for p in &pools {
        ...
        expanded.push(quote! {
            ...
            impl#impl_generics crndm::clone::PClone<#p> for #name #ty_generics #where_clause {
                #[inline]
                fn pclone(&self, j: &crndm::stm::Journal<#p>) -> Self {
                    #sum
                }
            }
        });
...
fn pools(attrs: Vec<Attribute>) -> Vec<TokenStream> {
    ...
    if p.is_empty() {
        vec![quote!{ crndm::default::BuddyAlloc }]
    } else {
        p
    }
}
    ...
    for param in &mut generics.params {
        if let GenericParam::Type(ref mut type_param) = *param {
            ...
            if !pool.iter().any(|p| p.to_string() == me) {
                type_param.bounds.push(parse_quote!(crndm::clone::PClone<#p>));
            }
        }
    }
    generics
            ...
            match data.fields {
                Fields::Named(ref fields) => {
                    let recurse = fields.named.iter().map(|f| {
                        ...
                        quote_spanned! {f.span()=>
                            #name: crndm::clone::PClone::pclone(&self.#name, j)
                        }
                    });
                    quote! {
                        Self {
                ...
                Fields::Unnamed(ref fields) => {
                    ...
                    let recurse = fields.unnamed.iter().enumerate().map(|(i, f)| {
                        ...
                        quote_spanned! {f.span()=>
                            crndm::clone::PClone::pclone(&self.#index, j)
                        }
                    });
                    quote! {
                        Self(#(#recurse,)*)
                    ...
                    Fields::Unnamed(ref fields) => {
                        ...
                        quote! {
                            #ident::#variant(#(#recurse,)*) => 
                                #ident::#variant(#(crndm::clone::PClone::pclone(&#clones, j),)*)
                        }
                    },
                    Fields::Named(ref fields) => {
                        ...
                        let clones = fields.named.iter().enumerate().map(|(i,f)| {
                            ...
                            let varname = format_ident!("__self_{}", i);
                            quote_spanned! {f.span()=>
                                #name: crndm::clone::PClone::pclone(&#varname, j)
                            }
                        });
                        quote! {
                            #ident::#variant{#(#recurse,)*} => 
