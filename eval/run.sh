...
if $all || $pmdk; then
    ...
    CPMEM_NO_CLWB=1 PMEM_NO_CLFLUSHOPT=$nofopt PMEM_NO_MOVNT=1 PMEM_NO_FLUSH=0 perf stat -a -C 0 -o $dir_path/outputs/perf/pmdk-bst-INS.out -d taskset -a -C 0 $dir_path/pmdk/btree $pool s 30000
    echo "Running performance test (PMDK-BST:CHK)..."
    CPMEM_NO_CLWB=1 PMEM_NO_CLFLUSHOPT=$nofopt PMEM_NO_MOVNT=1 PMEM_NO_FLUSH=0 perf stat -a -C 0 -o $dir_path/outputs/perf/pmdk-bst-CHK.out -d taskset -a -C 0 $dir_path/pmdk/btree $pool r 30000

    rm -f $pool
    pmempool create obj --layout=simplekv -s 1G $pool
    echo "Running performance test (PMDK-KVStore:PUT)..."
    CPMEM_NO_CLWB=1 PMEM_NO_CLFLUSHOPT=$nofopt PMEM_NO_MOVNT=1 PMEM_NO_FLUSH=0 perf stat -a -C 0 -o $dir_path/outputs/perf/pmdk-kv-PUT.out -d taskset -a -C 0 $dir_path/pmdk/simplekv $pool burst put 65536
    echo "Running performance test (PMDK-KVStore:GET)..."
    CPMEM_NO_CLWB=1 PMEM_NO_CLFLUSHOPT=$nofopt PMEM_NO_MOVNT=1 PMEM_NO_FLUSH=0 perf stat -a -C 0 -o $dir_path/outputs/perf/pmdk-kv-GET.out -d taskset -a -C 0 $dir_path/pmdk/simplekv $pool burst get 65536


    rm -f $pool
    for i in ${ins[@]}; do
    echo "Running performance test (PMDK-B+Tree:$i)..."
    PMEM_NO_CLWB=1 PMEM_NO_CLFLUSHOPT=$nofopt PMEM_NO_MOVNT=1 PMEM_NO_FLUSH=0 perf stat -a -C 0 -o $dir_path/outputs/perf/pmdk-$i.out -d taskset -a -C 0 $dir_path/pmdk/pmdk-1.8/src/examples/libpmemobj/map/mapcli btree $pool < $dir_path/inputs/perf/$i > /dev/null
    done
fi
...
if $all || $atlas; then
    ...
    perf stat -a -C 0 -o $dir_path/outputs/perf/atlas-bst-INS.out -d taskset -a -C 0 $dir_path/atlas/Atlas/runtime/build/tests/data_structures/btree s 30000
    echo "Running performance test (Atlas-BST:CHK)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/atlas-bst-CHK.out -d taskset -a -C 0 $dir_path/atlas/Atlas/runtime/build/tests/data_structures/btree r 30000

    echo "Running performance test (Atlas-KVStore:PUT)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/atlas-kv-PUT.out -d taskset -a -C 0 $dir_path/atlas/Atlas/runtime/build/tests/data_structures/simplekv burst put 65536
    echo "Running performance test (Atlas-KVStore:GET)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/atlas-kv-GET.out -d taskset -a -C 0 $dir_path/atlas/Atlas/runtime/build/tests/data_structures/simplekv burst get 65536

    rm -rf /mnt/pmem0/`whoami`  # Static in the code
    for i in ${ins[@]}; do
        echo "Running performance test (Atlas-B+Tree:$i)..."
        perf stat -C 0-4 -o $dir_path/outputs/perf/atlas-$i.out -d taskset -a -C 0 $dir_path/atlas/Atlas/runtime/build/tests/data_structures/btree_map < $dir_path/inputs/perf/$i > /dev/null
    done
fi
...
if $all || $mnemosyne; then
    ...
    perf stat -a -C 0 -o $dir_path/outputs/perf/mnemosyne-bst-INS.out -d taskset -a -C 0 ./build/examples/btree/btree s 30000
    echo "Running performance test (Mnemosyne-BST:CHK)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/mnemosyne-bst-CHK.out -d taskset -a -C 0 ./build/examples/btree/btree r 30000

    echo "Running performance test (Mnemosyne-KVStore:PUT)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/mnemosyne-kv-PUT.out -d taskset -a -C 0 ./build/examples/simplekv/simplekv burst put 65536
    echo "Running performance test (Mnemosyne-KVStore:GET)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/mnemosyne-kv-GET.out -d taskset -a -C 0 ./build/examples/simplekv/simplekv burst get 65536

    rm -rf /mnt/pmem0/psegments
    for i in ${ins[@]}; do
        echo "Running performance test (Mnemosyne-B+Tree:$i)..."
        perf stat -C 0-4 -o $dir_path/outputs/perf/mnemosyne-$i.out -d taskset -a -C 0 ./build/examples/btree_map/btree_map < $dir_path/inputs/perf/$i > /dev/null
    done
    cd $dir_path
fi
...
if $all || $go; then
    ...
    perf stat -a -C 0 -o $dir_path/outputs/perf/go-bst-INS.out -d taskset -a -C 0 $dir_path/go/btree $pool s 30000
    echo "Running performance test (go-pmem-BST:CHK)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/go-bst-CHK.out -d taskset -a -C 0 $dir_path/go/btree $pool r 30000

    rm -f $pool
    echo "Running performance test (go-pmem-KVStore:PUT)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/go-kv-PUT.out -d taskset -a -C 0 $dir_path/go/simplekv $pool burst put 65536
    echo "Running performance test (go-pmem-KVStore:GET)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/go-kv-GET.out -d taskset -a -C 0 $dir_path/go/simplekv $pool burst get 65536

    rm -f $pool
    for i in ${ins[@]}; do
    echo "Running performance test (go-pmem-B+Tree:$i)..."
    perf stat -a -C 0 -o $dir_path/outputs/perf/go-$i.out -d taskset -a -C 0 $dir_path/go/btree_map $pool < $dir_path/inputs/perf/$i > /dev/null
    done
fi
...
if $all || $crndm; then
    ...
    CPUS=1 perf stat -a -C 0 -o $dir_path/outputs/perf/crndm-bst-INS.out -d taskset -a -C 0 $dir_path/../target/release/examples/btree $pool s 30000
    echo "Running performance test (Corundum-BST:CHK)..."
    CPUS=1 perf stat -a -C 0 -o $dir_path/outputs/perf/crndm-bst-CHK.out -d taskset -a -C 0 $dir_path/../target/release/examples/btree $pool r 30000

    rm -f $pool
    echo "Running performance test (Corundum-KVStore:PUT)..."
    CPUS=1 perf stat -a -C 0 -o $dir_path/outputs/perf/crndm-kv-PUT.out -d taskset -a -C 0 $dir_path/../target/release/examples/simplekv $pool burst put 65536
    echo "Running performance test (Corundum-KVStore:GET)..."
    CPUS=1 perf stat -a -C 0 -o $dir_path/outputs/perf/crndm-kv-GET.out -d taskset -a -C 0 $dir_path/../target/release/examples/simplekv $pool burst get 65536

    rm -f $pool
    for i in ${ins[@]}; do
    echo "Running performance test (Corundum-B+Tree:$i)..."
    CPUS=1 perf stat -a -C 0 -o $dir_path/outputs/perf/crndm-$i.out -d taskset -a -C 0 $dir_path/../target/release/examples/mapcli btree $pool < $dir_path/inputs/perf/$i > /dev/null
    done
fi
