...
pub fn derive_root(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    for p in &pools {
        ...
        expanded.push(quote! {
            ...
            impl#impl_generics crndm::RootObj<#p> for #name #ty_generics #where_clause {
                #[inline]
                fn init(j: &crndm::stm::Journal<#p>) -> Self {
                    #sum
                }
            }
        });
    ...
    for param in &mut generics.params {
        if let GenericParam::Type(ref mut type_param) = *param {
            ...
            if !pool.iter().any(|p| p.to_string() == me) {
                type_param.bounds.push(parse_quote!(crndm::RootObj<#p>));
            }
        }
    }
    generics
                    ...
                    Fields::Unnamed(ref fields) => {
                        ...
                        quote! {
                            #ident::#variant(#(#recurse,)*) => 
                                #ident::#variant(#(crndm::RootObj::init(&#ints, j),)*)
                        }
                    },
                    Fields::Named(ref fields) => {
